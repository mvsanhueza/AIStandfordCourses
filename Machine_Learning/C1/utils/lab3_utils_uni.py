...
from utils.lab3_utils_common import compute_cost
from utils.lab3_utils_common import dlblue, dlorange, dldarkred, dlmagenta, dlpurple, dlcolors
...
def plt_ituition(x_train, y_train):
    ...
    @interact(w=(*w_range,10), continous_update=False)
    def func (w=150):
        f_wb = np.dot(x_train, w) + tmp_b

        fig, ax = plt.subplots(1,2,contrained_layout = True, figsize=(8,4))
...
class plt_update_onclick:
    def __init__(self, fig, ax, x_train, y_train, dyn_items):
        ...
        self.dyn_items = fig.canvas.mpl_connect('button_press_event', self)
...
def inbounds(a,b,xlim,ylim):
    ...
    if(ax > xlow and ax < xhigh) and (bx > xlow and bx < xhigh) and (ay > ylow and ay < yhigh) and (by > ylow and by < yhigh) : 
        return True
    return False

def plt_contour_wgrad(x,y,hist,ax,w_range=[-100,500,5], b_range = [-500,500,5], contours = [0.1,50,1000,5000,10000,25000,50000], resolution=5, w_final=200, b_final = 100, step = 10): 
